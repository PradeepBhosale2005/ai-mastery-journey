"""Shared LLM factory for LangChain Day-2 assignments.
...
- openai: OpenAI LangChain wrapper
...
def _extract_text_from_response(response_json: Dict[str, Any]) -> str:
    ...
    raise ValueError("Could not extract model text from API response.")
...
def get_llm(temperature: float = 0.2) -> Any:
    ...
    raise ValueError(
        f"Unsupported LLM_PROVIDER={provider!r}. Use company, ollama, openai, or gemini."
    )


def provider_name() -> str:
    return os.getenv("LLM_PROVIDER", "company").strip().lower()
