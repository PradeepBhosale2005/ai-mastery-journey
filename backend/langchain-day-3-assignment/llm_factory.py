"""Shared LLM factory for LangChain Day-3 assignments.

Configuration is read from .env using python-dotenv. No passwords or API keys
are hardcoded. The assignment scripts can run in deterministic local mode by
leaving USE_LIVE_LLM=false.
"""
...
def use_live_llm(default: bool = False) -> bool:
    """Return whether scripts should call a real LLM."""
    return parse_bool(os.getenv("USE_LIVE_LLM"), default=default)
...
def get_llm(temperature: float = 0.2) -> Any:
    ...
    provider = os.getenv("LLM_PROVIDER", "company").strip().lower()
    ...
    if provider == "company":
        ...
        return CompanyChatModel(
            ...
            verify_ssl=parse_bool(os.getenv("COMPANY_VERIFY_SSL"), default=True),
    ...
    if provider == "gemini":
        ...
        return ChatGoogleGenerativeAI(model=os.getenv("GEMINI_MODEL", "gemini-1.5-flash"), temperature=temperature)
